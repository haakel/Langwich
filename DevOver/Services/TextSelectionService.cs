...
public sealed class TextSelectionService : ITextSelectionService
{
    ...
    // نام کلاس پنجره‌های DevOver برای جلوگیری از ارسال کلید به خودمان
    private const string DevOverWindowClassName = "HwndWrapper[DevOver;;]";
    ...
    {
        // اگر پنجره خود DevOver فوکوس باشد، کاری نکن
        if (IsDevOverFocused()) return null;
    ...
    private static bool IsDevOverFocused()
    {
        try
        {
            ...
            // اگر پنجره DevOver باشد، کلید را ارسال نکن
            return className.Contains("HwndWrapper") && className.Contains("DevOver");
        }
        catch { return false; }
    }
